...
from timelesseg.typing import ToIterableInt
from timelesseg.models.MedNeXT.blocks import (
    MedNeXtBlock,
    MedNeXtDownBlock,
    MedNeXtUpBlock,
    OutBlock
...
class MedNeXt(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_0 = OutBlock(nn.ConvTranspose3d, in_channels=n_channels, n_classes=n_classes)

        # Used to fix PyTorch checkpointing bug
        self.dummy_tensor = nn.Parameter(torch.tensor([1.]), requires_grad=True)

        if deep_supervision:
            self.out_1 = OutBlock(nn.ConvTranspose3d, in_channels=n_channels*2, n_classes=n_classes)
            self.out_2 = OutBlock(nn.ConvTranspose3d, in_channels=n_channels*4, n_classes=n_classes)
            self.out_3 = OutBlock(nn.ConvTranspose3d, in_channels=n_channels*8, n_classes=n_classes)
            self.out_4 = OutBlock(nn.ConvTranspose3d, in_channels=n_channels*16, n_classes=n_classes)
...
if __name__ == "__main__":
    ...
    x = torch.zeros((1,1,64,64,64), requires_grad=False).to('mps')
