...
# This has been yanked from:
# https://github.com/MIC-DKFZ/batchgenerators/blob/78a621a7a1505180f5a333602cbefd2ddaccadd2/batchgenerators/dataloading/multi_threaded_augmenter.py


def producer(queue: Queue, data_loader, transform, thread_id: int, seed,
             abort_event, wait_time: float = 0.02):
...
def pin_memory_of_all_eligible_items_in_dict(result_dict: dict) -> dict:
...
def results_loop(in_queue: Queue, out_queue: thrQueue, abort_event,
                 pin_memory: bool, worker_list: list[Process],
                 gpu: int | None = None, wait_time: float = 0.02):
...
class NonDetMultiThreadedAugmenter:
    """
    ...
    def __init__(self, data_loader, transform, num_processes, num_cached=2, seeds=None, pin_memory=False,
        ...
        if isinstance(data_loader, DataLoader):
            assert data_loader.infinite, "Only use DataLoader instances that have infinite=True"
